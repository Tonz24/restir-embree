...
Texture::Texture( const char * file_name, TextureInterp interpType, TextureClamp clampType) : interpType(interpType), clampType(clampType){
	// image format
...
glm::vec3 Texture::get_texel( const int x, const int y ) const{	
...
glm::vec3 Texture::get_texel(const glm::vec2& uv) const{
	switch (interpType){
	case NEAREST:
...
glm::vec3 Texture::getTexelNearestNeighbor(const glm::vec2& uv) const{
...
glm::vec3 Texture::getTexelBilinear(const glm::vec2& uv) const {
